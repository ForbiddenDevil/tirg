...
def load_dataset(opt):
  ...
  elif opt.dataset == "VisualGenome":
    trainset=datasets.VisualGenome(
      path=opt.dataset_path,
      split='train',
      transform=torchvision.transforms.Compose([
        torchvision.transforms.Resize(224),
        torchvision.transforms.CenterCrop(224),
        torchvision.transforms.ToTensor(),
        torchvision.transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.255])
      ])
    )
    testset=datasets.VisualGenome(
      path=opt.dataset_path,
      split='test',
      transform=torchvision.transforms.Compose([
        torchvision.transforms.Resize(224),
...
def create_model_and_optimizer(opt, texts):
  ...
  if opt.model == 'imgonly':
    ...
    #print('main tirg --- ', len(texts), type(texts), texts[0])
...
def main():
  ...
  #print('Trianset: ', trainset[100], trainset[101])
